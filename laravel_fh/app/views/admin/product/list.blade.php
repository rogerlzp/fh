	...
	<div class="row">
		<div class="col-lg-12">
			<div class="page-header">
			  ...
			  <a data-toggle="modal" href="#add_category" class="btn btn-primary btn-lg">
			  {{ trans('admin.add_new_product') }}</a></span></h1>
			</div>
			   ...
			   <tbody id="sortable">
			    ...
			    <tr rel="{{ $product->id }}">
			        ...
			        <td>
				        ...
				        <a class="btn btn-primary btn-sm" href="{{url('admin/product/view/'.$product->id)}}">{{ trans('admin.edit') }}</a>
				        <a class="delete_toggler btn btn-danger btn-sm" rel="{{$product->id}}">{{ trans('admin.delete') }}</a>
			        	</div>
			        </td>
			     </tr>
			    @endforeach
