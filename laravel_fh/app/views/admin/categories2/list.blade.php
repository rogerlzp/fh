	...
	<div class="row">
		<div class="col-lg-12">
			<div class="page-header">
			  <h1>{{ trans('admin.category_for_tricks') }} <span class="pull-right"> 
			...
			<table class="table">
			   ...
			   <tbody id="sortable">
			    ...
			    <tr rel="{{ $category->id }}">
			        <td><a href="{{url('admin/category2/view/'.$category->id)}}">{{ $category->name }}</a></td>
			        <td>{{ $category->description}}<br>
			        </td>
			       
			        <td>
			        	<div class="btn-group pull-right">
				        <a class="btn btn-primary btn-sm" href="{{url('admin/category2/view/'.$category->id)}}">{{ trans('admin.edit') }}</a>
	...
	$(document).ready(function(){
		...
		$( "#sortable" ).sortable({
		  update: function(event, ui) {
		    ...
		    $.ajax({
		     type: 'POST',
		     url: "{{url('admin/category2/arrange')}}",
		     data: { data: order },
		    });
		    }
		});
		...
		$('.delete_toggler').each(function(index,elem) {
		    $(elem).click(function(e){
		    	e.preventDefault();
		    	var href = "{{url('admin/category2/delete')}}/";
