		...
		<div class="col-lg-12">
			...
			<table class="table">
			   ...
			   <tbody id="sortable">
			    ...
			    <tr rel="{{ $role->id }}">
			        <td><a href="{{url('admin/role/view/'.$role->id)}}">{{ $role->name }}</a></td>
			        <td> 
			        	<div class="btn-group pull-right">
				        <a class="btn btn-primary btn-sm" href="{{url('admin/role/view/'.$role->id)}}">{{ trans('admin.edit') }}</a>
				        <a class="delete_toggler btn btn-danger btn-sm" rel="{{$role->id}}">{{ trans('admin.delete') }}</a>
				         <a class="dbtn btn-primary btn-sm" href="{{url('admin/roleview/'.$role->id)}}">{{ trans('admin.security') }}</a>
			        	</div>
			        </td>
			     </tr>
			    @endforeach
