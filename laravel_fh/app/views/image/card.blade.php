<div href="#" class="trick-card col-lg-4 col-md-6 col-sm-6 col-xs-12">
	<div class="trick-card-inner js-goto-trick" data-slug="{{ $image->image_name }}">
		<a class="trick-card-title" href="{{ route('tricks.show', [ $trick->slug ]) }}">
			{{{ $image->description }}}
		</a>
		<div class="trick-card-stats trick-card-by">by <b><a href="{{ route('user.image_profile', $image->user->username) }}">{{ $image->user->username }}</a></b></div>
