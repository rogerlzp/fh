...
class TagRepository extends AbstractRepository implements TagRepositoryInterface
{
    ...
    /**
     * Find all tags with the associated number of activities.
     *
     * @return \Illuminate\Database\Eloquent\Collection|\Activity\Tag[]
     */
    public function findAllWithActivityCount()
    {
        return $this->model
                    ->leftJoin('activity_tag', 'tags.id', '=', 'activity_tag.tag_id')
                    ->leftJoin('activity', 'activity.id', '=', 'activity_tag.activity_id')
                    ->groupBy('tags.slug')
